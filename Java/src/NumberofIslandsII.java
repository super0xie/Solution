...
public class NumberofIslandsII {
    ...
    public List<Integer> numIslands2(int m, int n, int[][] positions) {
        ...
        for(int i = 0; i < positions.length; i++) {
            ...
            if(r > 0) {
                int upIndex = coorToIndex(r-1, c);
                if(taken[upIndex]) {
                    if(getRoot(uf, upIndex) != getRoot(uf, index)) {
                        union(uf, upIndex, index);
                        numOfIsland--;
                    }
                }
            }
            ...
            if(r < m-1) {
                int downIndex = coorToIndex(r+1, c);
                if(taken[downIndex]) {
                    if(getRoot(uf, downIndex) != getRoot(uf, index)) {
                        union(uf, downIndex, index);
                        numOfIsland--;
                    }
                }
            }
            ...
            if(c > 0) {
                int leftIndex = coorToIndex(r, c-1);
                if(taken[leftIndex]) {
                    if(getRoot(uf, leftIndex) != getRoot(uf, index)) {
                        union(uf, leftIndex, index);
                        numOfIsland--;
                    }
                }
            }
            ...
            if(c < n-1) {
                int rightIndex = coorToIndex(r, c+1);
                if(taken[rightIndex]) {
                    if(getRoot(uf, rightIndex) != getRoot(uf, index)) {
                        union(uf, rightIndex, index);
                        numOfIsland--;
                    }
                }
            }
